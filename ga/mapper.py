...
def as_net(solution):
    ...
    net.graph.add_edges_from(
        [(Population.index_to_feature[i[0]], Population.index_to_feature[i[1]]) for i in edges_as_indexes])
    # net.plot()
    return net
...
def pass_family_scores_between(new, old_nets):
    for node, parents in new.get_families().items():
        new_family_key = new.key(node, parents)

        for net in old_nets:
            if new_family_key in net.score_per_family.keys():
                new.score_per_family[new_family_key] = net.score_per_family[new_family_key]
                break
